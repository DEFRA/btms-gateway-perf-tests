...
# Run the test suite
jmeter -Djava.net.useSystemProxies=true -Jjmeterengine.force.system.exit=true ${JVM_PROXY_OPTS} -n -t ${SCENARIOFILE} -e -l "${REPORTFILE}" -o ${JM_REPORTS} -f -Jenv="${ENVIRONMENT}" -JbaseDir="${JM_HOME}" -j ${LOGFILE} #-j /dev/stdout 
test_exit_code=$?
